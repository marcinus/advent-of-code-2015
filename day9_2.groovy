...
static long longestHamiltonCycle(Map<String, Map<String, Long>> d, List<String> current) {
    ...
    d[current.last()].keySet().collect { i ->
        if (!visited.contains(i)) {
            return longestHamiltonCycle(d, current + i)
        }
        0L
    }.max() as long ?: 0L
}
