...
long solve(List<String> lines, long time = 1000) {
    ...
    reindeer.each { r -> r.points = 0 }
    (1..time).each {
        def reindeerToPoints = reindeer.collect { r ->
            long totalTime = r.timeFly + r.timeRest
            long fullCycles = (it / totalTime) as long
            long partial = it % totalTime
            if (partial > r.timeFly) {
                partial = r.timeFly
            }
            r.currentDistance = (fullCycles * r.timeFly + partial) * r.velocity
        }
        def maxCurrentDistance = reindeer.collect { it.currentDistance }.max()
        reindeer.findAll { it.currentDistance == maxCurrentDistance }.each { r ->
            r.points += 1
        }
    }
    reindeer.collect { it.points }.max()
}


assert 689L == solve(sampleInput.split('\n') as List<String>)
